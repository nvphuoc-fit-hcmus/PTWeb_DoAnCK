...
const Profile = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      alert('Loi khi cap nhat profile!');
    }
  };

  if (isLoading) {
    return <div className="loading">Dang tai thong tin...</div>;
  }

  return (
    <div style={{ maxWidth: '800px', margin: '0 auto' }}>
      <h1 style={{ marginBottom: '30px' }}>👤 Ho so ca nhan</h1>
          ...
          <div style={{ flex: 1 }}>
            {isEditing ? (
              <>
                ...
                <div className="form-group">
                  <label className="form-label">Gioi thieu</label>
                  <textarea
                    className="form-input"
                    rows="3"
                    value={formData.bio}
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  <button className="btn btn-secondary" onClick={() => setIsEditing(false)}>Huy</button>
                </div>
              </>
            ) : (
              <>
                <h2 style={{ marginBottom: '8px' }}>{profile?.display_name}</h2>
                <p style={{ color: 'var(--text-muted)', marginBottom: '8px' }}>@{profile?.username}</p>
      ...
      <div style={{
        ...
        {[
          { label: 'Tong game', value: profile?.stats?.total_games || 0, icon: '🎮' },
          { label: 'Thang', value: profile?.stats?.wins || 0, icon: '🏆' },
          { label: 'Thua', value: profile?.stats?.losses || 0, icon: '💔' },
          { label: 'Thanh tuu', value: profile?.stats?.achievements || 0, icon: '⭐' },
        ].map((stat) => (
      ...
      {/* Achievements */}
      <div className="card">
        <h3 style={{ marginBottom: '20px' }}>⭐ Thanh tuu</h3>
        {achievements.length > 0 ? (
          ...
          <p style={{ color: 'var(--text-muted)' }}>Chua mo khoa thanh tuu nao. Hay choi game de mo khoa!</p>
        )}
      </div>
    </div>
  );
